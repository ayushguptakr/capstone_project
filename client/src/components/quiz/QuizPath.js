    ...
    <div className="py-6 px-2 w-full max-w-[280px] mx-auto">
      <div className="relative flex flex-col items-center gap-6">
        ...
        {quizzes.map((quiz, index) => {
          ...
          else if (isLocked || unavailableBySchedule) nodeStyle = "bg-slate-100 border-slate-200 text-slate-300";
          ...
          return (
            <div key={quiz._id} className="relative z-10 flex flex-col items-center group w-full">
              <motion.button
                whileHover={isLocked || unavailableBySchedule ? {} : { scale: 1.1 }}
                whileTap={isLocked || unavailableBySchedule ? {} : { scale: 0.95 }}
                onClick={() => { if (!isLocked && !unavailableBySchedule) onPlay(quiz._id); }}
                style={{ x: xOffset }}
                className={`relative w-16 h-16 rounded-full border-b-4 flex items-center justify-center transition-all cursor-${isLocked || unavailableBySchedule ? 'not-allowed' : 'pointer'} ${nodeStyle} ${(!isCurrent && !isLocked && !unavailableBySchedule) ? 'opacity-80 hover:opacity-100' : ''}`}
              >
                {isLocked ? (
